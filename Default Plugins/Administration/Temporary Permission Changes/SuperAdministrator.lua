...
command.Shorthand = {"superadmin"}
command.Params = {"PlayerList"}
command.Usage = "superadmin Player"
command.Description = [[Gives the player temporary super-administrator permissions.]]
...
command.Run = function(main,user,players,...)
	...
	for _,player in pairs(players) do
		if SyncAPI.GetPermissionLevel(user) > SyncAPI.GetPermissionLevel(player) then
			...
			SyncAPI.SetPermissionLevel(player,3,false)
		else
