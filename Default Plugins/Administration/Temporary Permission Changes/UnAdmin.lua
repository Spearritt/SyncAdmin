...
command.Shorthand = {"unmod","unsuperadmin"}
command.Params = {"PlayerList"}
command.Usage = "unadmin Player"
command.Description = [[Temporarily sets the user's permission level to a regular user.]]
...
command.Run = function(main,user,players,...)
	...
	for _,player in pairs(players) do
		if SyncAPI.GetPermissionLevel(user) > SyncAPI.GetPermissionLevel(player) then
			...
			SyncAPI.SetPermissionLevel(player,0,false)
		else
