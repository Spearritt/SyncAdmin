...
command.Shorthand = {"pmod"}
command.Params = {"PlayerList"}
command.Usage = "pmod Player"
command.Description = [[Gives the player permanent moderator permissions.]]
...
command.Run = function(main,user,players,...)
	...
	for _,player in pairs(players) do
		if SyncAPI.GetPermissionLevel(user) > SyncAPI.GetPermissionLevel(player) then
			...
			SyncAPI.SetPermissionLevel(player,1,true)
		else
			return false,"You cannot run this command on someone with a higher permission level than you."
		end
	end
	
	return true,"Gave these players moderator permissions: " .. table.concat(list,", ")
end
